...
*/
/******************************************************************************
 *	@details	Constructor for rotary encoder. Provide the names of up and down
 *				actions such as 1, and 2, or 'U' and 'D', arduino pins for
 *				channels A and B, and number of detent per rotation. Please
 *				define the shaft click as a regular phi_buttons or
 *				phi_button_arrays object.
 *	@param		na - the name of (or pointer to) a char array that stores the
 *				names corresponding to the rotary encoder dial up and down.
 *	@param		ChnA - the arduino pin connected to the encoder channel A.
 *	@param		ChnB - the arduino pin connected to the encoder channel B.
 *	@param		det - the number of detents per rotation.
 *
 *	Example:	This rotary encoder returns U for up and D for down rotation
 *				on the dial.  Replace ChnA, ChnB, detent with actual numbers.
 *
 *				char mapping[]={'U','D'};
 *
 *				phi_rotary_encoders my_encoder(mapping, ChnA, ChnB, detent);
 *****************************************************************************/
...
/******************************************************************************
 *	@details	This actually performs the encoder read and returns up or down
 *				dials with the translation done by key_names.  If you are not
 *				very interested in the inner working of this library, this is
 *				the only function you need to call to get a response on the
 *				rotary encoder.  It assumes the channels are off when the knob
 *				is in a groove. To assume the channels are on when the knob is
 *				in a groove, read the code on stat_deq.  To properly sense the
 *				encoder, call this function inside of a loop.
 *
 *	@return		the named keys defined by the constructor (such as 'U' and 'D')
 *****************************************************************************/
byte phi_rotary_encoders::getKey()
{
	// For always on switches use {0,1,3,2,0,1,3,2,0}; For the sake of simple
	// coding, please don't mix always-on encoders with always-off encoders.
